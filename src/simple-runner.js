...
function loadConfig() {
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      console.log(`${colors.yellow}Warning: Invalid config file${colors.reset}`);
    }
  }
}
...
class SimpleRunner {
  ...
  handleKeyPress(str, key) {
    ...
    switch (key.name) {
      ...
      case 'a':
        ...
        this.log(`🔄 Auto-start ${config.autoStart ? 'enabled' : 'disabled'}`, 'info');
        this.render();
        break;
    }
  }
  ...
  async getTasks() {
    try {
      ...
      const tasks = [];
      const files = fs.readdirSync(config.backlogPath)
        .filter(file => file.match(/^task-\d+.*\.md$/))
        .sort();

      for (const file of files) {
        const content = fs.readFileSync(path.join(config.backlogPath, file), 'utf8');
        const task = this.parseTask(content, file);

        if (task.status === config.todoColumn || (!task.status && config.todoColumn === 'For Agent')) {
          tasks.push(task);
        }
      }
...
`;
    ...
    return new Promise((resolve, reject) => {
      ...
      child.stdout.on('data', (data) => {
        ...
        if (lines.length > 0) {
          this.log(`OpenCode: ${lines[lines.length - 1].substring(0, 80)}...`, 'info');
        }
      });
  ...
  async processTask(task) {
    ...
    try {
      ...
      if (task.isRevision && task.branch) {
        this.log(`🔄 Switching to branch: ${task.branch}`, 'info');
        this.exec(`git checkout ${task.branch}`);
      } else {
        this.log(`🌿 Creating branch: ${branchName}`, 'info');
        this.exec(`git checkout ${config.mainBranch}`);
        this.exec(`git pull origin ${config.mainBranch} || true`);
      ...
      // Push branch
      this.log('⬆️ Pushing to remote...', 'info');
      this.exec(`git push -u origin ${branchName}`);

      // Create PR if new task
      if (!task.isRevision) {
        try {
          this.exec(`gh pr create --title "feat: ${task.title}" --body "${task.description}" --head ${branchName} --base ${config.mainBranch}`);
          this.log('📋 Pull request created', 'success');
        } catch (error) {
          this.log('⚠️ PR creation failed (gh CLI not configured?)', 'warning');
        }
      }
  ...
  async render() {
    ...
    // Tasks
    const tasks = await this.getTasks();
    console.log(`${colors.bright}📋 Task Queue (${tasks.length}):${colors.reset}`);

    if (tasks.length === 0) {
      console.log(`${colors.gray}  No tasks in "${config.todoColumn}" column${colors.reset}`);
    } else {
      tasks.slice(0, 5).forEach((task, i) => {
        const prefix = i === 0 ? `${colors.green}▶${colors.reset}` : `${colors.gray}${i + 1}.${colors.reset}`;
        const revision = task.isRevision ? ` ${colors.red}(REV)${colors.reset}` : '';
        console.log(`  ${prefix} ${task.title}${revision}`);
      });

      if (tasks.length > 5) {
        console.log(`${colors.gray}  ... and ${tasks.length - 5} more${colors.reset}`);
      }
    }
    console.log();
    ...
    console.log(`  Runtime: ${minutes}m • Completed: ${colors.green}${this.stats.completed}${colors.reset} • Errors: ${colors.red}${this.stats.errors}${colors.reset}`);
    console.log();
